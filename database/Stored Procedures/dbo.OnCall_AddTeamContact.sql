...
-- ------------------------------------------------------------------------
-- Stored Procedure:  [OnCall_AddTeamContact]
--
----------------------------------------------------------------------------------
--COPYRIGHT (C) 2007 Quicken Loans Inc.
----------------------------------------------------------------------------------
--OnCall_AddTeamContact STORED PROCEDURE CREATION SCRIPT
----------------------------------------------------------------------------------
--
-- Revision History:
--
...
CREATE PROCEDURE [dbo].[OnCall_AddTeamContact]
@TeamID INTEGER,
@ContactID INTEGER
...
BEGIN TRY
	...
	INSERT INTO [OnCallTeamContacts]
           ([TeamID], [ContactID], [CreateDate], [LastUpdateDate])
     VALUES
           (@TeamID, @ContactID, getdate(), getdate())
...
BEGIN CATCH
   ...
   --SET @errorMessage = 'Error in executing OnCall_AddTeamContact. '
...
RETURN
GO
GRANT EXECUTE ON  [dbo].[OnCall_AddTeamContact] TO [AllAccess]
GO
