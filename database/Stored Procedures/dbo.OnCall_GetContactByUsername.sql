...
-- ------------------------------------------------------------------------
-- Stored Procedure:  [OnCall_GetContactByUsername]
--
----------------------------------------------------------------------------------
--COPYRIGHT (C) 2007 Quicken Loans Inc.
----------------------------------------------------------------------------------
--OnCall_GetContactByUsername STORED PROCEDURE CREATION SCRIPT
----------------------------------------------------------------------------------
--
-- Revision History:
--
...
CREATE PROCEDURE [dbo].[OnCall_GetContactByUsername]
@Username VARCHAR(50)
...
BEGIN TRY
	...
	SELECT con.[ContactID], con.[FirstName], con.[LastName],con.[EMail],
			con.[WorkPhone],null as [CellPhone], con.[HomePhone], con.[PreferredContactType], con.[Username],
			con.[StartTime], con.[EndTime], con.[OffHoursPreferredContactType], con.[OnCallAdmin]
		FROM [OnCallContacts] con	
		WHERE
			con.[Username] LIKE @Username
...
BEGIN CATCH
   ...
   --SET @errorMessage = 'Error in executing OnCall_GetContactByUsername. '
...
RETURN
GO
GRANT EXECUTE ON  [dbo].[OnCall_GetContactByUsername] TO [AllAccess]
GO
