...
-- ------------------------------------------------------------------------
-- Stored Procedure:  [OnCall_GetPreviousSchedule]
--
----------------------------------------------------------------------------------
--COPYRIGHT (C) 2007 Quicken Loans Inc.
----------------------------------------------------------------------------------
--OnCall_GetPreviousSchedule STORED PROCEDURE CREATION SCRIPT
----------------------------------------------------------------------------------
--
-- Revision History:
--
...
CREATE PROCEDURE [dbo].[OnCall_GetPreviousSchedule]
@TeamID INTEGER,
@RoleID INTEGER,
@ContactID INTEGER,
@Date DATETIME
...
BEGIN TRY
	...
	SELECT [ScheduleID], [TeamID], [StartDate], [EndDate], [RoleID], [ContactID], [RecurranceID]
	 ...
	 WHERE [TeamID] = @TeamID AND [RoleID] = @RoleID AND [ContactID] = @ContactID AND [EndDate] = @Date AND [RecurranceID] IS NULL
...
BEGIN CATCH
   ...
   --SET @errorMessage = 'Error in executing OnCall_GetPreviousSchedule. '
...
RETURN
GO
GRANT EXECUTE ON  [dbo].[OnCall_GetPreviousSchedule] TO [AllAccess]
GRANT EXECUTE ON  [dbo].[OnCall_GetPreviousSchedule] TO [OnCall]
GO
