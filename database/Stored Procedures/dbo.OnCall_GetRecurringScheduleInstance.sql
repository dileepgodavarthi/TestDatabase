...
-- ------------------------------------------------------------------------
-- Stored Procedure:  [OnCall_GetRecurringScheduleInstance]
--
----------------------------------------------------------------------------------
--COPYRIGHT (C) 2007 Quicken Loans Inc.
----------------------------------------------------------------------------------
--OnCall_GetRecurringScheduleInstance STORED PROCEDURE CREATION SCRIPT
----------------------------------------------------------------------------------
--
-- Revision History:
--
...
CREATE PROCEDURE [dbo].[OnCall_GetRecurringScheduleInstance]
...
BEGIN TRY
	...
	-- Validate Schedule
	IF (@RecurranceID IS NULL)
	BEGIN
		RAISERROR('The RecurranceID must be provided in OnCall_GetRecurringScheduleInstance!',16,1)
		RETURN -1
	END

	SELECT [ScheduleID], [TeamID], [StartDate], [EndDate], [RoleID], [ContactID], [RecurranceID]
	  FROM [OnCallSchedules]
	 WHERE [RecurranceID] = @RecurranceID AND [StartDate] = @StartTime
END TRY
...
BEGIN CATCH
   ...
   --SET @errorMessage = 'Error in executing OnCall_GetRecurringScheduleInstance. '
...
RETURN
GO
GRANT EXECUTE ON  [dbo].[OnCall_GetRecurringScheduleInstance] TO [AllAccess]
GRANT EXECUTE ON  [dbo].[OnCall_GetRecurringScheduleInstance] TO [OnCall]
GO
