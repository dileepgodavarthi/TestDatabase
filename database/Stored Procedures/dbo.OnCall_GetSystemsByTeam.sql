...
-- ------------------------------------------------------------------------
-- Stored Procedure:  [OnCall_GetSystemsByTeam]
--
----------------------------------------------------------------------------------
...
-- ------------------------------------------------------------------------
--
-- Description:	This procedure is used to all Systems associated with a team
-- ------------------------------------------------------------------------

----------------------------------------------------------------------------------

CREATE PROCEDURE [dbo].[OnCall_GetSystemsByTeam]
@TeamID INTEGER
...
BEGIN TRY
	...
	IF (@TeamID IS NULL)
	BEGIN
		RAISERROR('The TeamID must be provided in OnCall_GetSystemsByTeam!',16,1)
		RETURN -1
	END

	SELECT ocs.[SystemID], [Name], [TeamID], [PrimaryTechID]
	  FROM [OnCallSystems] ocs
		INNER JOIN [OnCallTeamSystems] octs ON ocs.[SystemID] = octs.[SystemID]
	  WHERE octs.[TeamID] = @TeamID
	  ORDER BY [Name]
END TRY
...
BEGIN CATCH
   ...
   --SET @errorMessage = 'Error in executing OnCall_GetSystemsByTeam. '
...
RETURN
GO
GRANT EXECUTE ON  [dbo].[OnCall_GetSystemsByTeam] TO [AllAccess]
GRANT EXECUTE ON  [dbo].[OnCall_GetSystemsByTeam] TO [OnCall]
GO
