...
CREATE PROCEDURE [dbo].[OnCall_GetTeamByID]
@TeamID INTEGER
...
BEGIN TRY
	...
	SELECT [TeamID], [Name], [OnCallEmail], [OnCallNumber], [OnCallExternalNumber], 
			[AlternateContact1], [AlternateContact2], [AlternateContact3], [PrimaryRole], 
			[Hidden], [EmailDefault], [TeamLeader], [EmailReminderText]
	  FROM [OnCallTeams]
      WHERE [TeamID]=@TeamID
...
GO
GRANT EXECUTE ON  [dbo].[OnCall_GetTeamByID] TO [AllAccess]
GO
