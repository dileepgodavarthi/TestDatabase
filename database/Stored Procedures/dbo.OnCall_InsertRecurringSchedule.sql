...
-- ------------------------------------------------------------------------
-- Stored Procedure:  [OnCall_InsertRecurringSchedule]
--
----------------------------------------------------------------------------------
--COPYRIGHT (C) 2007 Quicken Loans Inc.
----------------------------------------------------------------------------------
--OnCall_InsertRecurringSchedule STORED PROCEDURE CREATION SCRIPT
----------------------------------------------------------------------------------
--
-- Revision History:
--
...
CREATE PROCEDURE [dbo].[OnCall_InsertRecurringSchedule]
@TeamID INTEGER,
@ContactID INTEGER,
@RoleID INTEGER,
@StartTime DATETIME,
...
BEGIN TRY
	...
	INSERT INTO [OnCallRecurrance]
			   ([TeamID], [ContactID], [RoleID], [StartTime], [Duration], 
					[Type], [SubType], [Count], [DayMask], [CreateDate], [LastUpdateDate])
		 VALUES
			   (@TeamID, @ContactID, @RoleID, @StartTime, @Duration, 
					@Type, @SubType, @Count, @DayMask, getdate(), getdate())
...
BEGIN CATCH
   ...
   --SET @errorMessage = 'Error in executing OnCall_InsertRecurringSchedule. '
...
RETURN
GO
GRANT EXECUTE ON  [dbo].[OnCall_InsertRecurringSchedule] TO [AllAccess]
GRANT EXECUTE ON  [dbo].[OnCall_InsertRecurringSchedule] TO [OnCall]
GO
